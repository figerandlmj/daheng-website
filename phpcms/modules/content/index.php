...
class index {
	...
	public function get_province() {
		...
		if ($province) {
			...
			$this->db->query("SELECT DISTINCT to_province_id, to_province_name FROM " .  $this->db->db_tablepre . "city_price WHERE to_province_name LIKE '%" . $province . "%'");
	...
	public function get_city() {
		...
		if ($province_id && $city) {
			...
			$this->db->query("SELECT to_city_id, to_city_name FROM " .  $this->db->db_tablepre . "city_price WHERE to_province_id = " . $province_id . " AND to_city_name LIKE '%" . $city . "%'");
	...
	public function  getprovince(){
		...
		$data =$this->db->select_region($where = '1',$data = 'to_province_id,to_province_name' ,$limit = '', $order = 'id DESC', $group = 'to_province_id', $key='');
	...
	public function getcity(){
		...
		$id = $_GET['id'] = intval($_GET['id']);//获取省id

		$data =$this->db->select_region($where = 'to_province_id = '.$id ,$data = 'to_city_name,item_send_price,item_self_price,lowest_price' ,$limit = '', $order = 'id DESC', $group = '', $key='');
	...
	public function count_price(){
		...
		if ($_SERVER['REQUEST_METHOD'] == "POST") {
			...
			$province_id  = isset($_POST['to_province']) ? $_POST['to_province'] : '';
			$city_name  = isset($_POST['to_city']) ? $_POST['to_city'] : '';
			$price['weight']  = isset($_POST['weight']) ? $_POST['weight'] : '';//获取重量
			$price['volume'] = isset($_POST['volume']) ? $_POST['volume'] : '';//获取体积
			$type  = isset($_POST['type']) ? $_POST['type'] : '';//获取派送类型，1 自提，0 派送

			$data =$this->db->get_price($where = "to_province_id = '" . $province_id . "' AND to_city_name = '" . $city_name . "'" ,$data = 'to_province_name,to_city_name,item_send_price,item_self_price,lowest_price,lowest_self_price' ,$limit = '', $order = '', $group = '', $key='');
// exit(json_encode($data));
			//体积与重量换算，取最大的值为计费重量
			$new_weight = $price['volume']*230;
			if($new_weight>$price['weight']){
				$price['new_weight'] = $new_weight;
			}else{
				$price['new_weight'] = $price['weight'];
			}
			...
			// if($type ==1){
			// 	$price['send_price'] = $data['item_self_price'];//自提价格
			// }
			// else{
			// 	$price['send_price'] = $data['item_self_price']+$data['item_send_price'] ;//派送价格 = 数据库中的自提价格+派送价格
			// }

			//计算价格
			// $total_price = $price['send_price']*$price['new_weight'];

			//计算出的价格与最低价格比较，取二者最高

			// if($total_price>$data['lowest_price']){
			...
			// $price['to_province'] = $province_id;
			$price['to_province_name'] = $data['to_province_name'];
			$price['to_city'] = $city_name;
			$price['lowest_price'] = $data['lowest_price'];
			$price['lowest_self_price'] = $data['lowest_self_price'];


			//将用户提交的记录存入数据库
			$info['to_province_name'] = $price['to_province_name'];
			$info['to_city_name']=$price['to_city'];
			...
			$data = $this->db->insert_online($info);


			$price['standard'] = "芜湖到".$price['to_province_name'].$price['to_city']."最低自提费为".$price['lowest_self_price']."元,最低派送费为".$price['lowest_price']."元";
