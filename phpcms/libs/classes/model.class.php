...
class model {
	...
	//获取省市区
	final public function select_region($where = '', $data = '*', $limit = '', $order = '', $group = '', $key='') {
		if (is_array($where)) $where = $this->sqls($where);
		return $this->db->select($data, 'dh_city_price', $where, $limit, $order, $group, $key);
	}

	//获取一条价格信息
	final public function get_price($where = '', $data = '*', $order = '', $group = '') {
		if (is_array($where)) $where = $this->sqls($where);
		return $this->db->get_one($data, 'dh_city_price', $where, $order, $group);
	}
...
//	//查询价格信息
//	final public function list_price($where = '', $order = '', $page = 1, $pagesize = 20, $key='', $setpages = 10,$urlrule = '',$array = array(), $data = '*') {
//
//		$where = to_sqls($where);
//
//		$this->number = $this->count($where);
...
//	}
	...
	final public function insert_online($data, $return_insert_id = false, $replace = false) {

		return $this->db->insert($data, 'dh_online_price', $return_insert_id, $replace);
	}
